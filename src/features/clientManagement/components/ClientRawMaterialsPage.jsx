...
const ClientRawMaterialsPage = () => {
    ...
    const fetchClientDetails = async () => {
      if (client_id) {
        try {
          ...
          const response = await ClientService.get_client_by_id(client_id);

          console.log("🔍 Raw response from getClientById:", response);
    
          // If your API wraps the data under `data`, unwrap it
          ...
          if (clientData) {
            ...
            console.warn("⚠️ Aucune donnée client reçue");
            notification.warning({
              message: "Aucune donnée trouvée pour ce client"
            });
          }
        } catch (error) {
    ...
    const fetch_materials = async () => {
      ...
      try {
        ...
        const formattedMaterials = response.map((material) => ({
          ...
          remaining_quantity:
            material.remaining_quantity !== undefined
        ...
        notification.error({
          message: "Erreur",
          description:
            "Impossible de récupérer la liste des matières premières.",
        });
        set_loading(false);
      }
    };
  ...
  const handlePrintBill = () => {
    ...
    // Total quantité
    const total = billableData.reduce(
    ...
    window.print();
    notification.success({
      message: "Facture téléchargée et impression lancée",
    });
  };
  ...
  const handleSaveBill = async () => {
    try {
      ...
      const response = await ClientMaterialService.createMaterialInvoice(
        invoiceData
      );
      ...
      notification.error({
        message: "Erreur",
        description:
          error.message || "Échec lors de la sauvegarde du bon de livraison",
      });
    }
  };
  ...
  const handle_submit = async (values) => {
    try {
      ...
      if (editing_material) {
        ...
        new_material = await RawMaterialService.update_material(
          editing_material.id,
          {
            ...values,
        ...
        new_material.display_type = getMaterialTypeLabel(
          new_material.type_matiere
        );
        set_materials(
        ...
        // Add new material
        new_material = await RawMaterialService.add_material_to_client(
          client_id_int,
          {
            ...values,
            reception_date: values.reception_date.format("YYYY-MM-DD"),
      ...
      >
        <Form form={form} layout="vertical" onFinish={handle_submit}>
          ...
          <Form.Item>
            ...
            >
              <Button onClick={handle_cancel}>Annuler</Button>

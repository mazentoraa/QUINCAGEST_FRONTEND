...
export const ProductProvider = ({ children }) => {
  ...
  const addProduct = async (productData) => {
    try {
      ...
      if (typeof dataToSend === 'object') {
        if (dataToSend.price !== undefined) dataToSend.price = parseFloat(dataToSend.price) || 0;
        if (dataToSend.thickness !== undefined) dataToSend.thickness = parseFloat(dataToSend.thickness) || 0;
        if (dataToSend.length !== undefined) dataToSend.length = parseFloat(dataToSend.length) || 0;
        if (dataToSend.surface !== undefined) dataToSend.surface = parseFloat(dataToSend.surface) || 0;
      }
  ...
  const updateProduct = async (id, productData) => {
    try {
      ...
      // Create product model and set the ID
      const product = new ProductModel(dataToSend);
      product.id = id;

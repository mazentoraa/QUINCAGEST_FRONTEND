...
import "../../installments/components/InstallmentDetails.css";
import SupplierInstallmentService from "../Services/SupplierInstallmentService";

const SupplierInstallmentDetails = ({ installment, onBack, onUpdateInstallment }) => {
    console.log('installment:', installment);

  // Filtrage renforcé des doublons côté affichage (sécurité supplémentaire)
  const mappedInstallmentDetails = useMemo(() => {
    const today = new Date();
    // Filtrer les doublons par id OU montant+date
    const seen = new Set();
    const uniqueTraites = (installment.traites || []).filter(tr => {
    ...
    const sortedTraites = [...uniqueTraites].sort((a, b) => new Date(a.date_echeance) - new Date(b.date_echeance));
  ...
  const globalStatus = calculateGlobalStatus(mappedInstallmentDetails);
  ...
  // Redéfinition de getStatusLabel (nécessaire pour l'affichage)
  ...
  const handleInstallmentStatusChange = (index, newStatus) => {
    ...
    const updatedInstallmentDetails = mappedInstallmentDetails.map((detail, i) => ({
      ...
      status: updatedStatuses[i] === "PAYEE" ? "PAYEE" : "NON_PAYEE",
    }));
    ...
    if (typeof onUpdateInstallment === 'function') {
  ...
  const handleBulkStatusChange = (newStatus) => {
    ...
    const updatedInstallmentDetails = mappedInstallmentDetails.map((detail) => ({
      ...detail,
      // Correction : status toujours PAYEE ou NON_PAYEE
      status: upperStatus === "PAYEE" ? "PAYEE" : "NON_PAYEE",
    }));
    const globalStatus = calculateGlobalStatus(updatedInstallmentDetails);
    const updatedInstallment = {
      ...installment,
        ...
        <button
          ...
          onClick={() => {
            if (typeof onBack === 'function') {
              ...
              alert("Erreur : la fonction de retour n'est pas définie dans le parent !");
            }
          }}
        >
          ← Retour à la liste
        </button>
        <h1>Détails des Traites - {installment.fournisseur_nom || installment.supplierName || installment.nom_raison_sociale || ''}</h1>
      </div>

      <div className="client-info-card">
        <h2>Informations Fournisseur</h2>
        <div className="info-grid">
          <div className="info-item"><label>Nom du Fournisseur:</label><span>{installment.fournisseur_nom || installment.supplierName || installment.nom_raison_sociale || ''}</span></div>
          <div className="info-item"><label>Adresse:</label><span>
  {installment.fournisseur?.adresse 
    || installment.fournisseurAdresse 
    || "Non spécifiée"}
</span>
</div>
          <div className="info-item"><label>Numéro de Facture:</label><span>{installment.numero_facture || installment.numero_commande || ''}</span></div>
          <div className="info-item"><label>Montant Total:</label><span className="amount">{installment.montant_total || installment.totalAmount || ''} DT</span></div>
        </div>
      </div>

      <div className="installment-summary-cards">
        <div className="summary-card total"><h3>{mappedInstallmentDetails.length}</h3><p>TOTAL TRAITES</p></div>
        <div className="summary-card paid"><h3>{Object.values(installmentStatuses).filter((s) => s === "PAYEE").length}</h3><p>PAYÉES</p></div>
        <div className="summary-card unpaid"><h3>{Object.values(installmentStatuses).filter((s) => s === "NON_PAYEE").length}</h3><p>NON PAYÉES</p></div>
      ...
      <div className="installments-table-section">
        ...
        <div className="bulk-action-buttons">
          <button className="bulk-button mark-all-paid" onClick={() => handleBulkStatusChange("PAYEE")}>Marquer tout comme Payée</button>
          <button className="bulk-button mark-all-unpaid" onClick={() => handleBulkStatusChange("NON_PAYEE")}>Marquer tout comme Non Payée</button>
        </div>
      </div>
    </div>
  );
...
export default SupplierInstallmentDetails;

...
import BonRetourService from "./Services/BonRetourService";
...
const [fournisseurSearchLoading, setFournisseurSearchLoading] = useState(false);
...
}, [searchText, statusFilter, fournisseurFilter, dateFilter]);
...
if (fournisseurFilter) queryParams.fournisseur = fournisseurFilter;
...
const fetchFournisseurs = async () => {
  try {
    const data = await FournisseurService.getAll(); // Appelle l'API
    let list = [];
    if (data && Array.isArray(data.results)) {
      list = data.results; // Si paginé
    } else if (Array.isArray(data)) {
      list = data; // Si tableau simple
    }

    setFournisseurs(list); // Tu peux t’en servir si besoin ailleurs

    // Générer les options pour Select
    ...
    setFournisseurOptions([]); // Nettoie les options si erreur
  }
};

const handleSearch = async (value) => {
  if (value.length < 2) return;
  setFournisseurSearchLoading(true); // ✅ Correction ici
  try {
    const fournisseurs = await FournisseurService.search(value);
    setFournisseurOptions(
    ...
    message.error("Erreur lors de la recherche des fournisseurs");
  } finally {
    setFournisseurSearchLoading(false); // ✅ Correction ici
  }
};
...
console.log("Dates transformées en moment:", {
  ...
  form.setFieldsValue({
    ...
    fournisseur: recordWithDetails.fournisseur?.id || recordWithDetails.fournisseur,
    date_reception: recordWithDetails.date_reception
  ...
  // Chargement des matières disponibles du fournisseur
const fournisseurId =
  recordWithDetails.fournisseur?.id || recordWithDetails.fournisseur;
if (fournisseurId) {
  await handleFournisseurChange(fournisseurId);
}
...
const handleFournisseurChange = async (fournisseurId) => {
  ...
  try {
    const response = await BonRetourService.getAvailableMaterials(fournisseurId);
...
const generateBonRetourPDF = async (recordFromList) => {
  ...
  try {
    // Récupère les détails complets du bon de retour
    ...
    let fournisseurData = fullRecord.fournisseur;

    // Si le fournisseur n'est pas chargé ou incomplet, on le récupère via l'API
    if (
      !fournisseurData ||
      typeof fournisseurData !== "object" ||
      !fournisseurData.nom
    ) {
      const fournisseurId =
        fullRecord.fournisseur?.id ||
        fullRecord.fournisseur ||
        recordFromList.fournisseur?.id ||
        recordFromList.fournisseur;

      if (fournisseurId) {
        try {
          fournisseurData = await FournisseurService.getById(fournisseurId); // Service mis à jour
          fullRecord.fournisseur = fournisseurData;
        } catch (error) {
          console.warn("Could not fetch fournisseur data for PDF:", error);
        }
      }
    }

    // On s'assure que fournisseur est un objet valide
    if (
      typeof fullRecord.fournisseur !== "object" ||
      fullRecord.fournisseur === null
    ) {
      fullRecord.fournisseur = {
        nom: "N/A",
        adresse: "N/A",
    ...
    // Envoi des données au service de génération PDF
  ...
  key: "fournisseur",
  render: (text, record) =>
    record.fournisseur?.nom || record.fournisseur_name || "N/A",
},
    {
      title: "Date Retour",
...
>
  ...
  onClick={() => {
    setSearchText("");
    setStatusFilter("");
    setFournisseurFilter(""); // ✅ Correction ici
    setDateFilter("");
  }}
>
    Effacer les filtres
...
export default BonRetourFornisseur;

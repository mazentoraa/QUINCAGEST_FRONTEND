...
export default function Consommables() {
  ...
  const fetchConsommables = async () => {
    ...
    try {
      ...
      setConsommables(Array.isArray(response.data.results) ? response.data.results : response.data);
    } catch (error) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const payload = {
        ...values,
        prix_unitaire:
          values.prix_unitaire !== undefined && values.prix_unitaire !== null && values.prix_unitaire !== ""
            ? parseFloat(values.prix_unitaire)
            : null,
        quantite:
          values.quantite !== undefined && values.quantite !== null && values.quantite !== ""
